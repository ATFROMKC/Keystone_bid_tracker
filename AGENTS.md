...
3. `SESSION_NOTES.md`
4. `OTHER_PC_HANDOFF.md` (if present — short “what I did before push” for the other machine)
5. `NEXT_CHAT_CHECKLIST.md`
6. `.cursor/rules/context-first.mdc`
7. `.cursor/rules/git-session-sync.mdc`
...
4. `OTHER_PC_HANDOFF.md` (latest intent from whoever last pushed; overwrite each session—if it conflicts with code or `SESSION_NOTES.md`, trust code, then notes)
5. Legacy docs such as `docs/HANDOFF.md`
...
- **Session end:** when the user wraps up or asks to sync/push, run **`git status`**, stage, commit (user message or assistant-proposed summary), **push to GitHub**, and **append a dated entry to `SESSION_NOTES.md`** unless the user opts out of notes. When pushing meaningful multi-PC work, offer to refresh **`OTHER_PC_HANDOFF.md`** (see **`git-session-sync.mdc`**).
